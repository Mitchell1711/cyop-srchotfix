...
{
    //don't go back to the editor of editorpause is enabled
    if (global.editingLevel && (!global.editorpause))
    {
        room_goto(rmEditor)
        return;
    }
...
{
    //changed so it uses menu input binds instead
    src_menu_getinput()
    ...
    selected += moveselect
    //scroll over to the top of bottom of pause menu
        ...
        switch selected
        {
            ...
            case 3:
                if (room == Endingroom || room == Creditsroom || room == Johnresurrectionroom)
                    break
                //just go back to the editor room if you're exiting while in editormode
                else if global.editingLevel
                {
                    if (global.fromHub && global.hubLevel != "")
                        global.levelName = global.hubLevel
                    scr_pause_activate_objects()
                    room_goto(rmEditor)
